...
contract("GsnCapableDocumentStore", accounts => {
  ...
  describe("constructor", () => {
    ...
    it("sets the owner properly", async () => {
      const trustedForwarder = await instance.trustedForwarder();
  ...
  describe("version", () => {
    it("should have a version field value that should be bumped on new versions of the contract", async () => {
      ...
      expect(versionFromSolidity).to.be.equal("2.4.0");
    });

    it("should return version when versionRecipient called", async () => {
      const versionFromSolidity = await instance.versionRecipient();
      expect(versionFromSolidity).to.be.equal("2.4.0");
    });
  });
  ...
  describe("supportsInterface", () => {
    it("returns true if supportsInterface for GsnCapable", async () => {
      const supportsGsnCapableInterface = await instance.supportsInterface("0xa5a23640");
      expect(supportsGsnCapableInterface).to.be.true;
    });
