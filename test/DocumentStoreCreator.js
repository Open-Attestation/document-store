...
describe("DocumentStoreCreator", async () => {
  let Accounts;
  let UpgradeableDocumentStore;
  let DocumentStoreCreator;

  before("", async () => {
    Accounts = await ethers.getSigners();
    UpgradeableDocumentStore = await ethers.getContractFactory("UpgradeableDocumentStore");
  ...
  describe("deploy", () => {
    it("should deploy new instance of DocumentStore correctly", async () => {
      ...
      const deployedDocumentStore = await UpgradeableDocumentStore.attach(receipt.events[2].args.instance);
