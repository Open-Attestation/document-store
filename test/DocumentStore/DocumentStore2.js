...
describe("DocumentStore", async () => {
  ...
  describe("issue", () => {
    it("should be able to issue a document", async () => {
      ...
      const receipt = DocumentStoreInstance.issue(documentMerkleRoot);
    ...
    it("should not allow duplicate issues", async () => {
      ...
      await expect(DocumentStoreInstance.issue(documentMerkleRoot)).to.be.rejectedWith(/revert/, "Duplicate issue was not rejected");
    });
    ...
    it("only allows the owner to issue", async () => {
      ...
      // FIXME:
      // await expect(DocumentStoreInstance.issue(documentMerkleRoot, { from: nonOwner })).to.be.rejectedWith(/revert/);
    });
  });
  ...
  describe("bulkIssue", () => {
    it("should be able to issue one document", async () => {
      ...
      const receipt = await DocumentStoreInstance.bulkIssue(documentMerkleRoots);
    ...
    it("only allows the owner to issue", async () => {
      ...
      // FIXME:
      // await expect(DocumentStoreInstance.bulkIssue(documentMerkleRoots, { from: nonOwner })).to.be.rejectedWith(/revert/);
    });
  });
  ...
  describe("revoke", () => {
    it("should allow the revocation of a valid and issued document", async () => {
      ...
      const receipt = await DocumentStoreInstance.revoke(documentHash);
  ...
  describe("bulkRevoke", () => {
    it("should be able to revoke one document", async () => {
      const documentMerkleRoots = ["0x3a267813bea8120f55a7b9ca814c34dd89f237502544d7c75dfd709a659f6330"];
      const receipt = await DocumentStoreInstance.bulkRevoke(documentMerkleRoots);
    ...
    it("only allows the owner to revoke", async () => {
      ...
      // FIXME:
      // await expect(DocumentStoreInstance.bulkRevoke(documentMerkleRoots, { from: nonOwner })).to.be.rejectedWith(/revert/);
    });
  });
  ...
  describe("isRevokedBefore", () => {
    ...
    it("returns false for document revoked after the block number", async () => {
      const revokeReceipt = await DocumentStoreInstance.revoke(documentHash);
      // FIXME:
  ...
  describe("isIssuedBefore", () => {
    ...
    it("returns false for document issued after the block number", async () => {
      const issueReceipt = await DocumentStoreInstance.issue(documentHash);
      // FIXME:
