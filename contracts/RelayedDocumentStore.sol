 ...
 * @dev This contract enables meta-transactions to be sent and delegated to a third party.
  ...
  *       1. documentsHash: The hash of an array of documentMerkleRoots that are being bulk issued / revoked. Only for bulk operations.
  *                         - const message = web3.utils.soliditySha3(...documentMerkleRoots);
  *       2. signature: The off-chain signature, signed with the privateKey from the trustedSigner EOA, that will be used to
  ...
  *       3. Generate the signature.  This signature is generated by signing the hash of: the nonce, the message as well as the relay contract address.
  ...
  *             const nonce = RelayedDocumentStore.signerNonce(trustedSigner);
