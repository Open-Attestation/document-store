...
let account: string;
let deployedDocumentStoreCreatorAddress: string;
...
beforeAll(async () => {
  ...
  deployedDocumentStoreCreatorAddress = receipt.address;
  account = await signer.getAddress();
});

describe("deploy", () => {
  it("deploys a new DocumentStore contract without waiting for confirmation", async () => {
    const receipt = await deploy("My Store", signer, deployedDocumentStoreCreatorAddress);
...
describe("deployAndWait", () => {
  it("deploys a new DocumentStore contract", async () => {
    const instance = await deployAndWait("My Store", signer, deployedDocumentStoreCreatorAddress);
...
describe("connect", () => {
  it("connects to existing contract", async () => {
    const {address} = await deployAndWait("My Store", signer, deployedDocumentStoreCreatorAddress);
